...
//! plugin connections (Ninja, Level, Snipe-IT, Iru) for a given local
...
//! `commands::iru::CachedIruSyncDto`), instead of defining the JSON shape
...
//! them). Iru's own `collect_iru`/`read_iru_cache_file` below follow that
//! same established convention rather than reaching into `commands::iru`'s
//! private helpers, for consistency rather than because of that same
//! historical constraint (this module is free to touch `commands/iru.rs`,
