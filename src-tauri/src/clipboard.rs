...
pub fn read_image_as_png_base64() -> Result<(String, String), AppError> {
    let mut clipboard = Clipboard::new()
        .map_err(|e| AppError::Config(format!("Zwischenablage nicht verfügbar: {e}")))?;
    ...
    let buffer: ImageBuffer<Rgba<u8>, Vec<u8>> =
        ImageBuffer::from_raw(width, height, image_data.bytes.into_owned()).ok_or_else(|| {
            AppError::Config("Zwischenablage-Bild hat unerwartetes Format".to_string())
        })?;
