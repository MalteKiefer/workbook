...
/// on any line that also contains this IP" parse works identically across
/// all three without OS-specific branches. Returns an empty map (never an
/// error) if `arp` isn't on PATH, exits non-zero, or produces no parseable
/// entries -- MAC/vendor enrichment is always optional, never something a
/// scan can fail on.
