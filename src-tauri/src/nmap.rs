...
/// Runs `nmap -sV -T4 <target>` (version detection over a plain TCP
/// connect scan -- deliberately NOT `-sS`/`-O`, which need raw sockets
/// and elevated privileges on every platform this app targets) and
/// returns its combined stdout, or an error if nmap isn't installed or
/// exits non-zero. Output is shown to the admin as-is (see brief: no
/// structured parsing in this feature).
