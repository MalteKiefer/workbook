...
impl Plugin for DummyPlugin {
    ...
    fn list_systems(
        ...
        Ok(vec![
            ExternalSystem {
                external_id: "dummy-1".into(),
                name: "Dummy-Server".into(),
                hostname: Some("dummy-server.local".into()),
            },
            ExternalSystem {
                external_id: "dummy-2".into(),
                name: "Dummy-Firewall".into(),
                hostname: Some("dummy-fw.local".into()),
            },
        ])
    }
    ...
    fn get_system_details(
        ...
        Ok(
            serde_json::json!({ "external_id": external_id, "status": "ok", "note": "Beispieldaten der Attrappen-Implementierung" }),
        )
    }

    fn link_system(&self, local_system_id: i64, external_id: &str) -> Result<(), PluginError> {
        println!(
            "DummyPlugin: verknüpfe lokales System {local_system_id} mit externer ID {external_id}"
        );
        Ok(())
    }
}
