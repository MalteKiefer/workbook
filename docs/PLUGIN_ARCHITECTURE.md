...
Snipe-IT (`plugin::snipeit`, `commands::snipeit`) und Iru (`plugin::iru`,
...
als dünne Hülle um
`NinjaPluginSection.tsx`/`LevelPluginSection.tsx`/`SnipeitPluginSection.tsx`/`IruPluginSection.tsx`).
...
## Iru-Plugin (`plugin::iru`) -- vierte echte Integration
...
`PluginsView.tsx` bindet die Sektion als vierte Karte neben
