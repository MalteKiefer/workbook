...
(`plugin::acronis`, `commands::acronis`) und netcup (`plugin::netcup`,
`commands::netcup`), alle mit Mehrfach-Verbindungs-Unterstützung. Acronis
ist dabei anders als die übrigen vierzehn: kein RMM/MDM-Gerätebestand,
sondern Sicherungsstatus pro Gerät (siehe eigener Abschnitt unten).
...
## netcup-Plugin (`plugin::netcup`): fünfzehnte echte Integration
