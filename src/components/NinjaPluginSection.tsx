...
export default function NinjaPluginSection() {
  ...
  async function createAndLink(connection: NinjaConnectionDto, group: NinjaOrgDeviceGroupDto, device: ExternalSystemDto) {
    ...
    try {
      ...
      await invoke("sync_ninja_connection", { connectionId: connection.id }).catch(() => {});
      await loadCachedSync(connection);
    } catch (err) {
      setDeviceError((prev) => ({ ...prev, [key]: formatInvokeError(err) }));
    } finally {
  ...
  async function createAndLinkAll(connection: NinjaConnectionDto, group: NinjaOrgDeviceGroupDto, devices: ExternalSystemDto[]) {
