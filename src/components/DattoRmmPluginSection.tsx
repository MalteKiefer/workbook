...
// Datto RMM (cloud-hosted, multi-pod RMM, part of Kaseya,
// https://www.datto.com/product/rmm/) plugin settings screen — ninth
...
// Tactical RMM (TacticalRmmPluginSection.tsx, the closest structural
// template for the overall shape of this file — connection list, a
// per-connection "Sites & Geräte" overlay, collapsed-by-default groups,
// filterable/paginated device lists, j/k/Enter/l/u keyboard navigation,
// compare/adopt panel — see docs/PLUGIN_ARCHITECTURE.md "Datto-RMM-Plugin").
// Same read-only/manual-adopt rule as every plugin section here: external
// data is always shown as supplementary, read-only information next to a
// System's own maintained fields (name/hostname/ip_address/notes), and
// copying a value across is always a deliberate, manual, per-field action —
// never an automatic sync step.
//
// Important, verified difference from Tactical RMM (see
// docs/PLUGIN_ARCHITECTURE.md "Datto-RMM-Plugin" and plugin::dattormm's own
// module docs): Datto RMM's hierarchy is flat -- just "Site" -> "Device", no
// higher "Client" grouping level -- and a device's `siteUid` is a GENUINE
// foreign key to `Site.uid` (no name-join workaround needed, unlike Tactical
// RMM). So this file maps/groups a connection's devices by Site directly,
// structurally closer to JamfPluginSection.tsx's per-Site grouping than to
// TacticalRmmPluginSection.tsx's Client-level grouping — every "client_*"
// concept in the Tactical RMM template becomes a "site_*" concept here, one
// level, no second layer.
//
// Also unlike Tactical RMM: Datto RMM genuinely reports a per-device (and
// per-site) web-dashboard deep link (`portal_url`, a real, confirmed field —
// see module docs) — rendered here via <DattoRmmLink>, the same "In X
// öffnen" convention as NinjaPluginSection.tsx's ninja_url/
// SnipeitPluginSection.tsx's snipeit_url, just optional (Option<String>,
// since Datto RMM might not report one for every single device) instead of
// always-constructible from base_url.
//
// A connection authenticates with an API Key + a separate API Secret Key
// (not a single API key like Tactical RMM, not an OAuth2 client ID/secret
// pair like NinjaOne/Jamf) — see plugin::dattormm module docs on the
// specific `grant_type=password` OAuth token exchange this pair feeds into.
// Devices are fetched cache-first, exactly like every other plugin here:
// get_cached_dattormm_sync (offline, reads the last known sync) vs.
// sync_dattormm_connection (live, also refreshes the cache server-side). The
// cached sync result is also the single source of truth for Sites
// (site_uid/site_name/customer_id/portal_url come straight off each cached
// group) — no separate live "list sites" call from this page (the backend
// keeps list_dattormm_sites available for symmetry/first-time setup, but
// this page never calls it), so opening the overlay below without ever
// syncing simply shows a "Noch nicht synchronisiert" prompt.

interface DattoRmmConnectionDto {
  id: string;
  label: string;
  base_url: string;
}
...
interface ExternalSystemDto {
  ...
  // "online" | "offline", converted backend-side from Datto RMM's own real
  // JSON boolean `online` field (see plugin::dattormm module docs) — kept
  // as a free string here too, not a union type, so an unrecognized future
  // value still renders instead of failing a type check.
  status: string | null;
  // One of Datto RMM's own `deviceClass` values
...
interface DattoRmmSiteDeviceGroupDto {
  site_uid: string;
  site_name: string;
  // A genuine, confirmed web-dashboard deep link to this exact site.
  portal_url: string | null;
  customer_id: number | null;
  devices: ExternalSystemDto[];
}

interface CachedDattoRmmSyncDto {
...
// (see plugin::dattormm module docs) — a direct match to a local System's
// own fields, the same convention as every other plugin section here. Kept
// as a small candidate-key scan anyway (not a hardcoded single key), in case
// a future Datto RMM API version renames/adds a field.
const NAME_KEYS = ["hostname", "name"];
const HOSTNAME_KEYS = ["hostname"];
const IP_KEYS = ["intIpAddress", "extIpAddress", "ip_address", "ip"];
...
// Human-readable labels for Datto RMM's own status/platform values — an
// unrecognized value simply falls back to the raw string instead of hiding
// it, so a future Datto RMM release adding a new value degrades gracefully
// instead of showing nothing. Unlike Tactical RMM, Datto RMM only reports
// two status values (see plugin::dattormm module docs — no "overdue").
...
const PLATFORM_LABELS: Record<string, string> = {
  ...
  printer: "Drucker",
  esxihost: "ESXi-Host",
  rmmnetworkdevice: "Netzwerkgerät (RMM)",
  unknown: "Unbekannt",
};

// Heuristic scan of a get_dattormm_system_details() payload: the exact
...
// spirit to every other plugin section's findExternalValue.
...
// Devices come back from the Datto RMM API in whatever order the API
...
// OS default handler. Unlike NinjaLink/SnipeitLink/JamfLink (always a
// constructible, non-null URL), `url` here is genuinely optional — Datto
// RMM might not report a portalUrl for every device — so this renders
// nothing at all when it's absent, instead of a dead/placeholder link.
function DattoRmmLink({ url }: { url: string | null }) {
  if (!url) return null;
  return (
    <a
      href={url}
...
// Small colored dot + label for a device's online/offline status.
...
function DeviceSummaryLine({ device }: { device: ExternalSystemDto }) {
  return (
    <span style={{ display: "inline-flex", alignItems: "center", gap: "0.5rem", flexWrap: "wrap" }}>
      ...
      <DattoRmmLink url={device.portal_url} />
    </span>
  );
}

// Datto RMM devices DO have a real hostname field (it's RMM monitoring
// software, not asset/inventory management) — so, like Ninja/Level/Tactical
// RMM, the suggested match key for "Link to existing system" is simply the
// device's own hostname, compared against the only free-text field a local
// system has for it — System.hostname.
function matchKeyForDevice(device: ExternalSystemDto): string {
  return (device.hostname || "").trim().toLowerCase();
}

export default function DattoRmmPluginSection() {
  ...
  const [connections, setConnections] = useState<DattoRmmConnectionDto[]>([]);
  ...
  // siteMapBusy/siteMapError below (`${connectionId}:${siteUid}`). Set when
  ...
  // Site-mapping busy/error, keyed by `${connectionId}:${siteUid}`.
  ...
  const [cachedSyncByConnection, setCachedSyncByConnection] = useState<Record<string, CachedDattoRmmSyncDto | null>>({});
  ...
  // Device text filter, keyed by `${connectionId}:${siteUid}`.
  ...
  // Keyed by `${connectionId}:${siteUid}`, same as everything else below.
  // Deliberately NOT an accordion: multiple sites can be expanded at once,
  // mirroring every other plugin section here.
  const [expandedGroupKeys, setExpandedGroupKeys] = useState<Set<string>>(new Set());

  // Device-list pagination, 10 devices/page, keyed the same way
  // (`${connectionId}:${siteUid}`); 0-based, so 0 means "Seite 1".
  ...
  // mirrors the j/k/Enter pattern used by every other plugin section here.
  // Each site's "Nicht verknüpft" + "Bereits verknüpft" devices are treated
  // as ONE flat, ordered list (unlinked first, then linked — see
  // computeGroupDevices), and `activeGroupKey`
  // (`${connectionId}:${siteUid}`) says which site's list currently owns
  ...
  // Group-scoped, keyed by `${connectionId}:${site_uid}` — disables the
  ...
  const reloadConnections = useCallback(() => {
    invoke<DattoRmmConnectionDto[]>("list_dattormm_connections")
      .then(setConnections)
      ...
      void (async () => {
        try {
          ...
          if (newlyCreated.length === 1) {
            ...
            const siteUid = groupKey.slice(separatorIdx + 1);
            const connection = connections.find((c) => c.id === connectionId);
            const group = cachedSyncByConnection[connectionId]?.groups.find((g) => g.site_uid === siteUid);
            if (connection && group) {
  ...
  const loadCachedSync = useCallback(async (connection: DattoRmmConnectionDto) => {
    const id = connection.id;
    setCacheBusy((prev) => ({ ...prev, [id]: true }));
    setCacheError((prev) => ({ ...prev, [id]: null }));
    try {
      const cached = await invoke<CachedDattoRmmSyncDto | null>("get_cached_dattormm_sync", { connectionId: id });
  ...
  function openSiteDeviceModal(connection: DattoRmmConnectionDto) {
  ...
  async function handleTestConnection() {
    ...
    setTestResult(null);
    try {
      await invoke("test_dattormm_connection", {
        baseUrl: newBaseUrl,
        apiKey: newApiKey,
  ...
  async function handleAddConnection(e: React.FormEvent) {
    ...
    setAddBusy(true);
    try {
      await invoke("add_dattormm_connection", {
        label: newLabel,
        baseUrl: newBaseUrl,
        apiKey: newApiKey,
        apiSecretKey: newApiSecretKey,
  ...
  async function handleRemoveConnection(connection: DattoRmmConnectionDto) {
    const confirmed = window.confirm(`Datto-RMM-Verbindung "${connection.label}" wirklich entfernen?`);
    if (!confirmed) return;
    const id = connection.id;
    setRemoveBusy((prev) => ({ ...prev, [id]: true }));
    setConnectionsError(null);
    try {
      await invoke("remove_dattormm_connection", { id });
  ...
  async function handleSync(connection: DattoRmmConnectionDto) {
    const id = connection.id;
    setSyncBusy((prev) => ({ ...prev, [id]: true }));
    setSyncStatus((prev) => ({ ...prev, [id]: null }));
    setSyncError((prev) => ({ ...prev, [id]: null }));
    try {
      const groups = await invoke<DattoRmmSiteDeviceGroupDto[]>("sync_dattormm_connection", { connectionId: id });
  ...
  async function handleSiteMappingChange(
    connection: DattoRmmConnectionDto,
    group: DattoRmmSiteDeviceGroupDto,
    rawValue: string,
  ) {
    const mapKey = `${connection.id}:${group.site_uid}`;
    setSiteMapBusy((prev) => ({ ...prev, [mapKey]: true }));
    setSiteMapError((prev) => ({ ...prev, [mapKey]: null }));
    try {
      if (rawValue === "") {
        await invoke("unmap_dattormm_site", { connectionId: connection.id, siteUid: group.site_uid });
      } else {
        const customerId = Number(rawValue);
        await invoke("map_dattormm_site", {
          connectionId: connection.id,
          siteUid: group.site_uid,
        ...
        return {
          ...
          [connection.id]: {
            ...
            groups: current.groups.map((g) =>
              g.site_uid === group.site_uid ? { ...g, customer_id: rawValue === "" ? null : Number(rawValue) } : g,
            ),
          },
        };
      });
  ...
  function openLinkPicker(connection: DattoRmmConnectionDto, group: DattoRmmSiteDeviceGroupDto, device: ExternalSystemDto) {
  ...
  async function confirmLink(connection: DattoRmmConnectionDto, device: ExternalSystemDto) {
  ...
  async function createAndLink(connection: DattoRmmConnectionDto, group: DattoRmmSiteDeviceGroupDto, device: ExternalSystemDto) {
    if (group.customer_id === null) return;
    const customerId = group.customer_id;
    const key = `${connection.id}:${device.external_id}`;
    ...
    try {
      ...
      // rather than silently losing that Datto-RMM-native context on
      ...
      const noteLines = [
        `Datto-RMM-Site: ${group.site_name}`,
  ...
  async function createAndLinkAll(connection: DattoRmmConnectionDto, group: DattoRmmSiteDeviceGroupDto, devices: ExternalSystemDto[]) {
    if (group.customer_id === null || devices.length === 0) return;
    const customerId = group.customer_id;
    const groupKey = `${connection.id}:${group.site_uid}`;
  ...
  async function handleUnlink(connection: DattoRmmConnectionDto, device: ExternalSystemDto) {
    if (device.linked_system_id === null) return;
    const key = `${connection.id}:${device.external_id}`;
    setUnlinkBusy((prev) => ({ ...prev, [key]: true }));
    setDeviceError((prev) => ({ ...prev, [key]: null }));
    try {
      await invoke("unlink_system_from_dattormm", { systemId: device.linked_system_id, connectionId: connection.id });
  ...
  async function toggleDetails(connection: DattoRmmConnectionDto, group: DattoRmmSiteDeviceGroupDto, device: ExternalSystemDto) {
    const key = `${connection.id}:${device.external_id}`;
    ...
    try {
      const detailsPromise = invoke<Record<string, unknown>>("get_dattormm_system_details", {
        connectionId: connection.id,
        externalId: device.external_id,
      });
  ...
  function computeGroupDevices(connectionId: string, group: DattoRmmSiteDeviceGroupDto) {
    const groupKey = `${connectionId}:${group.site_uid}`;
  ...
  // pattern as every other plugin section here, so typing in the filter
  // input (or any other field) never fights with these single-letter
  // bindings. Only live while the site/device overlay is actually open.
  useEffect(() => {
    function onKeyDown(e: KeyboardEvent) {
      ...
      const activeGroup = activeGroupKey
        ? cached.groups.find((g) => `${openConnectionId}:${g.site_uid}` === activeGroupKey)
        : undefined;
  ...
  useEffect(() => {
    ...
    setActiveGroupKey((prev) => {
      if (prev && cached.groups.some((g) => `${openConnectionId}:${g.site_uid}` === prev)) return prev;
      const firstWithDevices = cached.groups.find((g) => g.devices.length > 0);
      return firstWithDevices ? `${openConnectionId}:${firstWithDevices.site_uid}` : null;
    });
  ...
  function renderDetailsPanel(group: DattoRmmSiteDeviceGroupDto, device: ExternalSystemDto, key: string) {
    ...
    return (
      ...
      >
        ...
        {busy && <p style={{ margin: 0, fontSize: "0.8rem", color: "var(--text-secondary)" }}>Lade Datto-RMM-Details…</p>}
              ...
              <table style={{ borderCollapse: "collapse", fontSize: "0.82rem", width: "100%" }}>
                <thead>
                  <tr>
                    ...
                    <th style={thStyle}>Datto RMM (extern)</th>
                    <th style={thStyle}></th>
                  </tr>
                </thead>
                <tbody>
  ...
  function renderDeviceGroup(connection: DattoRmmConnectionDto, group: DattoRmmSiteDeviceGroupDto, isFirstFilterable: boolean) {
    ...
    return (
      ...
      >
        ...
        >
          ...
          <DattoRmmLink url={group.portal_url} />
        </div>
          ...
          <select
            ...
            onChange={(e) => {
              ...
              if (raw === CREATE_NEW_CUSTOMER_VALUE) {
                ...
                // Remember which site is waiting so the
        ...
        {isExpanded && (
          <>
            ...
            {group.devices.length === 0 && <p style={mutedStyle}>Keine Geräte bei dieser Site.</p>}
  ...
  function renderConnectionModal(connection: DattoRmmConnectionDto) {
    const id = connection.id;
    ...
    return (
      <Modal onClose={closeSiteDeviceModal}>
        <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem", minWidth: "32rem", maxWidth: "60rem", maxHeight: "80vh", overflowY: "auto" }}>
          ...
          {!cacheBusyFlag && hasCacheLoaded && !cached && (
            ...
            >
              <p style={{ margin: 0, fontSize: "0.85rem", color: "var(--text-secondary)" }}>
                Noch nicht synchronisiert. Auf "Aktualisieren" klicken, um Sites und Geräte von Datto RMM zu holen.
              </p>
              <button type="button" disabled={busy} onClick={() => void handleSync(connection)}>
                {busy ? "Aktualisiere…" : "Aktualisieren"}
              </button>
          ...
          {cached &&
            (() => {
              const firstFilterableSiteUid = cached.groups.find((g) => g.devices.length > 0)?.site_uid ?? null;
              return cached.groups.map((group) => renderDeviceGroup(connection, group, group.site_uid === firstFilterableSiteUid));
            })()}
        </div>
      </Modal>
    );
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem", maxWidth: "52rem" }}>
      <section style={cardStyle}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: "0.5rem", flexWrap: "wrap" }}>
          <h2 style={{ margin: 0, fontSize: "1rem" }}>Datto-RMM-Verbindungen</h2>
          <button type="button" onClick={() => setAddFormOpen(true)}>
            + Neue Verbindung
          </button>
        </div>
        <p style={{ margin: 0, color: "var(--text-secondary)", fontSize: "0.85rem" }}>
          Liest ausschließlich Geräte aus Datto RMM (cloud-gehostetes RMM-Tool von Kaseya) — es werden nie Daten
          dorthin zurückgeschrieben. Eine Verbindung kann mehrere Sites sehen; jede Site wird einzeln einem Kunden
          zugeordnet.
        </p>
      ...
      {addFormOpen && (
        <Modal onClose={() => setAddFormOpen(false)}>
          <form onSubmit={(e) => void handleAddConnection(e)} style={{ display: "flex", flexDirection: "column", gap: "0.5rem", minWidth: "22rem" }}>
            <h2 style={{ margin: 0, fontSize: "1rem" }}>Neue Datto-RMM-Verbindung</h2>
            <label style={{ display: "flex", flexDirection: "column", gap: "0.2rem" }}>
              Label
              <input value={newLabel} onChange={(e) => setNewLabel(e.target.value)} placeholder="ACME Datto RMM" required autoFocus />
            </label>
            <label style={{ display: "flex", flexDirection: "column", gap: "0.2rem" }}>
              Base-URL
              <input
                ...
                placeholder="https://merlot-api.centrastage.net"
                style={{ fontFamily: "var(--font-mono)" }}
                required
              />
              <span style={{ fontSize: "0.75rem", color: "var(--text-muted)" }}>
                Die API-Adresse des eigenen, Pod-spezifischen Datto-RMM-Kontos (Datto RMM ist auf sechs Regionen/Pods
                verteilt — Pinotage, Merlot, Concord, Vidal, Zinfandel, Syrah). Zu finden auf der eigenen
                Datto-RMM-Nutzerseite, nachdem API-Keys erzeugt wurden — es gibt keine Möglichkeit, sie automatisch
                abzuleiten.
              </span>
            </label>
            <label style={{ display: "flex", flexDirection: "column", gap: "0.2rem" }}>
            ...
            <label style={{ display: "flex", flexDirection: "column", gap: "0.2rem" }}>
              ...
              <input
                ...
                onChange={(e) => {
                  setNewApiSecretKey(e.target.value);
                  setTestResult(null);
                }}
                required
              />
              <span style={{ fontSize: "0.75rem", color: "var(--text-muted)" }}>
                Erzeugt in Datto RMM unter Setup → Users → eigenen Nutzer wählen → Generate API Keys.
              </span>
            </label>
            ...
            <div style={{ display: "flex", gap: "0.5rem", justifyContent: "flex-end", marginTop: "0.25rem" }}>
              ...
              <button type="button" disabled={testBusy || !newBaseUrl || !newApiKey || !newApiSecretKey} onClick={() => void handleTestConnection()}>
