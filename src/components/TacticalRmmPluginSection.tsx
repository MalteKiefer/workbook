...
// Tactical RMM (self-hosted, open-source RMM, github.com/amidaware/tacticalrmm)
// plugin settings screen — fourth integration after NinjaOne
// (NinjaPluginSection.tsx), Level.io (LevelPluginSection.tsx), and Snipe-IT
// (SnipeitPluginSection.tsx, the structural template for this file — see
// docs/PLUGIN_ARCHITECTURE.md "Snipe-IT-Plugin", described there as the
// most mature version of this pattern). Same read-only/manual-adopt rule as
// all three: external data is always shown as supplementary, read-only
...
// deliberate, manual, per-field action — never an automatic sync step. That
// rule shapes the whole "Details/Aktualisieren" panel below.
//
// A connection is NOT tied to a single Kunde, mirroring Ninja/Snipe-IT
// rather than Level: Tactical RMM models a real "Client" > "Site" > "Agent"
// hierarchy inside one self-hosted instance, and a single connection (one
// Base-URL + API key) can see multiple Clients — a realistic MSP scenario is
// one shared Tactical RMM instance managing several customers' agents, each
// as its own Client. So each connection's Clients are individually mapped to
// a local Kunde, and agents are grouped/shown per-Client, gated on that
// Client being mapped — structurally identical to Ninja's/Snipe-IT's
// per-Organization/-Company mapping. Mapping granularity deliberately stops
// at the Client level, NOT Site — a Client can have multiple Sites, but this
// integration doesn't add a second mapping layer for those (see
// plugin::tacticalrmm module docs); an agent's Site is still shown as
// read-only, informational context (device.site_name).
//
// Important, verified difference from Ninja/Snipe-IT (see
// docs/PLUGIN_ARCHITECTURE.md "Tactical-RMM-Plugin" and
// plugin::tacticalrmm's own module docs): Tactical RMM's agent list API
// carries no numeric client ID at all, only a flat `client_name` string —
// the backend already resolves the client grouping via that name join
// before this component ever sees the data, so this file just consumes the
// resulting `TacticalRmmClientDeviceGroupDto[]` the same way
// SnipeitPluginSection.tsx consumes its company groups. Unlike Snipe-IT,
// Tactical RMM agents DO have real `hostname`/`ip_address` fields (it's RMM
// monitoring software, not asset/inventory management) — so, like Ninja/
// Level, `hostname` drives both the device summary line and the "existing
// system" match heuristic below, NOT an asset-tag-style fallback chain.
// Tactical RMM also reports `status` (online/offline/overdue) and
// `platform` (windows/linux/darwin) per agent, shown as small badges.
//
// There is deliberately no "open in Tactical RMM" link on a device row,
// unlike Ninja's/Snipe-IT's dashboard link: Tactical RMM's web dashboard is
// commonly deployed on its own subdomain/host, independent of the API's
// Base-URL used for this connection (see plugin::tacticalrmm module docs) —
// so there is no honest way to construct that link from data this app has.
//
// Agents are fetched cache-first, exactly like Ninja/Snipe-IT:
// get_cached_tacticalrmm_sync (offline, reads the last known sync) vs.
// sync_tacticalrmm_connection (live, also refreshes the cache server-side).
// The cached sync result is also the single source of truth for Clients
// (client_id/client_name/customer_id come straight off each cached group) —
// no separate live "list clients" call from this page (the backend keeps
// list_tacticalrmm_clients available for symmetry/first-time setup, but this
// page never calls it), so opening the overlay below without ever syncing
// simply shows a "Noch nicht synchronisiert" prompt.
//
// UI structure: the connections list stays inline on the page (identity + a
// couple of buttons per connection). Everything else — clients,
// Kunde-mapping, and the linked/unlinked device lists — lives behind a
// per-connection "Clients & Geräte" button that opens a Modal overlay,
// tracked via local `openConnectionId` state, matching Ninja's/Snipe-IT's
// overlay pattern exactly, including the collapsed-by-default client
// groups, keyboard-navigable (j/k/Enter/l/u) filtered device list, and
// 10-per-page pagination.

interface TacticalRmmConnectionDto {
  id: string;
  label: string;
  base_url: string;
}
...
interface ExternalSystemDto {
  ...
  // "online" | "offline" | "overdue", passed through verbatim from Tactical
  // RMM (see plugin::tacticalrmm module docs) — kept as a free string here
  // too, not a union type, so an unrecognized future value still renders
...
interface CachedTacticalRmmSyncDto {
...
// (verified against Tactical RMM's own AgentSerializer source, see
// plugin::tacticalrmm module docs) — a much more direct match to a local
// System's own fields than Snipe-IT's asset JSON needed. Kept as a small
// candidate-key scan anyway (not a hardcoded single key), the same
// convention as every other plugin section here, in case a future Tactical
// RMM version renames/adds a field.
...
// Human-readable labels for Tactical RMM's own free-form status/platform
// strings (see module docs on why they're not Rust/TS enums) — an
// unrecognized value simply falls back to the raw string instead of hiding
// it, so a future Tactical RMM release adding a new value degrades
// gracefully instead of showing nothing.
const STATUS_LABELS: Record<string, string> = { online: "Online", offline: "Offline", overdue: "Überfällig" };
const STATUS_COLORS: Record<string, string> = {
  online: "var(--success)",
  offline: "var(--text-muted)",
  overdue: "var(--danger)",
};
const PLATFORM_LABELS: Record<string, string> = { windows: "Windows", linux: "Linux", darwin: "macOS" };

// Heuristic scan of a get_tacticalrmm_system_details() payload: the exact
// shape isn't part of the fixed contract (it's just Record<string,
// unknown>), so we look for top-level, non-object fields whose key matches
// one of a handful of common name/hostname/ip-shaped key names. Identical in
// spirit to NinjaPluginSection.tsx's/SnipeitPluginSection.tsx's
// findExternalValue.
...
// Devices come back from the Tactical RMM API in whatever order the API
// returns them in — sort alphabetically by name (German collation, e.g. "ä"
// sorts near "a") for display, independent of the text filter.
function sortByName<T extends { name: string }>(items: T[]): T[] {
  return items.slice().sort((a, b) => a.name.localeCompare(b.name, "de", { sensitivity: "base" }));
}

// How many devices an expanded client shows per page.
...
// Small colored dot + label for an agent's online/offline/overdue status.
...
// client's device list. Hides itself entirely when there's only one page.
  ...
  return (
    <span style={{ display: "inline-flex", alignItems: "center", gap: "0.5rem", flexWrap: "wrap" }}>
      <span>
        ...
        {device.site_name && (
          <span style={{ color: "var(--text-secondary)", fontSize: "0.85em" }}> · {device.site_name}</span>
        )}
        {device.platform && (
...
// Tactical RMM agents DO have a real hostname field (it's RMM monitoring
// software, not asset/inventory management, unlike Snipe-IT) — so, like
// Ninja/Level, the suggested match key for "Link to existing system" is
// simply the device's own hostname, compared against the only free-text
// field a local system has for it — System.hostname.
function matchKeyForDevice(device: ExternalSystemDto): string {
  return (device.hostname || "").trim().toLowerCase();
}

export default function TacticalRmmPluginSection() {
  ...
  const [connections, setConnections] = useState<TacticalRmmConnectionDto[]>([]);
  const [customers, setCustomers] = useState<Customer[]>([]);
  const [connectionsError, setConnectionsError] = useState<string | null>(null);

  // Which client's Kunde-mapping select is waiting on the globally-mounted
  // CustomerForm modal to finish creating a customer, keyed the same way as
  // clientMapBusy/clientMapError below (`${connectionId}:${clientId}`). Set
  // when the user picks "+ Neuen Kunden anlegen…"; consumed and cleared by
  // the customerEditorTarget-closed effect further down, which then
  // auto-maps the newly created customer to this client.
  ...
  // Which connection's "Clients & Geräte" overlay is open. Page-local UI
  ...
  // Client-mapping busy/error, keyed by `${connectionId}:${clientId}`.
  const [clientMapBusy, setClientMapBusy] = useState<Record<string, boolean>>({});
  const [clientMapError, setClientMapError] = useState<Record<string, string | null>>({});

  // Cached (offline) device sync per connection.
  const [cachedSyncByConnection, setCachedSyncByConnection] = useState<Record<string, CachedTacticalRmmSyncDto | null>>({});
  ...
  // Device text filter, keyed by `${connectionId}:${clientId}`.
  const [deviceFilter, setDeviceFilter] = useState<Record<string, string>>({});

  // Which clients are expanded — collapsed by default (a client renders as
  // just a header row: name, Kunde-mapping select, and a device-count
  // summary). Keyed by `${connectionId}:${clientId}`, same as everything
  // else below. Deliberately NOT an accordion: multiple clients can be
  // expanded at once, mirroring Ninja's/Snipe-IT's organizations/companies.
  const [expandedGroupKeys, setExpandedGroupKeys] = useState<Set<string>>(new Set());

  // Device-list pagination, 10 devices/page, keyed the same way
  // (`${connectionId}:${clientId}`); 0-based, so 0 means "Seite 1".
  ...
  // JournalView.tsx. Each client's "Nicht verknüpft" + "Bereits verknüpft"
  // devices are treated as ONE flat, ordered list (unlinked first, then
  // linked — see computeGroupDevices), and `activeGroupKey`
  // (`${connectionId}:${clientId}`) says which client's list currently owns
  // the keyboard highlight — only one group is keyboard-active at a time;
  ...
  // Group-scoped, keyed by `${connectionId}:${clientId}` — disables the
  ...
  const reloadConnections = useCallback(() => {
    invoke<TacticalRmmConnectionDto[]>("list_tacticalrmm_connections")
      .then(setConnections)
  ...
  // anlegen…" on one of the client selects below (tracked via
  ...
  // customer, and — if exactly one appeared — auto-map it to the client
  // that was waiting, via the exact same handleClientMappingChange path
      ...
      void (async () => {
        try {
          ...
          if (newlyCreated.length === 1) {
            ...
            const clientId = groupKey.slice(separatorIdx + 1);
            const connection = connections.find((c) => c.id === connectionId);
            const group = cachedSyncByConnection[connectionId]?.groups.find((g) => g.client_id === clientId);
            if (connection && group) {
              await handleClientMappingChange(connection, group, String(newlyCreated[0].id));
            }
          }
  ...
  const loadCachedSync = useCallback(async (connection: TacticalRmmConnectionDto) => {
    const id = connection.id;
    setCacheBusy((prev) => ({ ...prev, [id]: true }));
    setCacheError((prev) => ({ ...prev, [id]: null }));
    try {
      const cached = await invoke<CachedTacticalRmmSyncDto | null>("get_cached_tacticalrmm_sync", { connectionId: id });
  ...
  function openClientDeviceModal(connection: TacticalRmmConnectionDto) {
  ...
  function closeClientDeviceModal() {
  ...
  async function handleTestConnection() {
    ...
    setTestResult(null);
    try {
      await invoke("test_tacticalrmm_connection", { baseUrl: newBaseUrl, apiKey: newApiKey });
  ...
  async function handleAddConnection(e: React.FormEvent) {
    ...
    setAddBusy(true);
    try {
      await invoke("add_tacticalrmm_connection", { label: newLabel, baseUrl: newBaseUrl, apiKey: newApiKey });
  ...
  async function handleRemoveConnection(connection: TacticalRmmConnectionDto) {
    const confirmed = window.confirm(`Tactical-RMM-Verbindung "${connection.label}" wirklich entfernen?`);
    if (!confirmed) return;
    const id = connection.id;
    setRemoveBusy((prev) => ({ ...prev, [id]: true }));
    setConnectionsError(null);
    try {
      await invoke("remove_tacticalrmm_connection", { id });
  ...
  async function handleSync(connection: TacticalRmmConnectionDto) {
    const id = connection.id;
    setSyncBusy((prev) => ({ ...prev, [id]: true }));
    setSyncStatus((prev) => ({ ...prev, [id]: null }));
    setSyncError((prev) => ({ ...prev, [id]: null }));
    try {
      const groups = await invoke<TacticalRmmClientDeviceGroupDto[]>("sync_tacticalrmm_connection", { connectionId: id });
      const deviceCount = groups.reduce((sum, g) => sum + g.devices.length, 0);
      setSyncStatus((prev) => ({
        ...prev,
        [id]: `${groups.length} Client(s), ${deviceCount} Gerät(e) gefunden.`,
      }));
      ...
      // The device list just changed underneath any expanded client — stale
      ...
      // this connection's clients back to page 1.
      setPageByGroup((prev) => {
        const next = { ...prev };
  ...
  async function handleClientMappingChange(
    connection: TacticalRmmConnectionDto,
    group: TacticalRmmClientDeviceGroupDto,
    rawValue: string,
  ) {
    const mapKey = `${connection.id}:${group.client_id}`;
    setClientMapBusy((prev) => ({ ...prev, [mapKey]: true }));
    setClientMapError((prev) => ({ ...prev, [mapKey]: null }));
    try {
      if (rawValue === "") {
        await invoke("unmap_tacticalrmm_client", { connectionId: connection.id, clientId: group.client_id });
      } else {
        const customerId = Number(rawValue);
        await invoke("map_tacticalrmm_client", {
        ...
        return {
          ...
          [connection.id]: {
            ...
            groups: current.groups.map((g) =>
              g.client_id === group.client_id ? { ...g, customer_id: rawValue === "" ? null : Number(rawValue) } : g,
            ),
          },
        };
      });
      ...
      setClientMapError((prev) => ({ ...prev, [mapKey]: formatInvokeError(err) }));
    } finally {
      setClientMapBusy((prev) => ({ ...prev, [mapKey]: false }));
    }
  }

  function openLinkPicker(connection: TacticalRmmConnectionDto, group: TacticalRmmClientDeviceGroupDto, device: ExternalSystemDto) {
  ...
  async function confirmLink(connection: TacticalRmmConnectionDto, device: ExternalSystemDto) {
  ...
  async function createAndLink(connection: TacticalRmmConnectionDto, group: TacticalRmmClientDeviceGroupDto, device: ExternalSystemDto) {
    if (group.customer_id === null) return;
    const customerId = group.customer_id;
    const key = `${connection.id}:${device.external_id}`;
    ...
    setDeviceError((prev) => ({ ...prev, [key]: null }));
    try {
      // A local System has no site/status/platform field of its own —
      // rather than silently losing that Tactical-RMM-native context on
      ...
      const noteLines = [
        device.site_name ? `Tactical-RMM-Site: ${device.site_name}` : null,
        device.platform ? `Plattform: ${PLATFORM_LABELS[device.platform] ?? device.platform}` : null,
  ...
  async function createAndLinkAll(connection: TacticalRmmConnectionDto, group: TacticalRmmClientDeviceGroupDto, devices: ExternalSystemDto[]) {
    if (group.customer_id === null || devices.length === 0) return;
    const customerId = group.customer_id;
    const groupKey = `${connection.id}:${group.client_id}`;
  ...
  async function handleUnlink(connection: TacticalRmmConnectionDto, device: ExternalSystemDto) {
    if (device.linked_system_id === null) return;
    const key = `${connection.id}:${device.external_id}`;
    setUnlinkBusy((prev) => ({ ...prev, [key]: true }));
    setDeviceError((prev) => ({ ...prev, [key]: null }));
    try {
      await invoke("unlink_system_from_tacticalrmm", { systemId: device.linked_system_id, connectionId: connection.id });
  ...
  async function toggleDetails(connection: TacticalRmmConnectionDto, group: TacticalRmmClientDeviceGroupDto, device: ExternalSystemDto) {
    const key = `${connection.id}:${device.external_id}`;
    ...
    try {
      const detailsPromise = invoke<Record<string, unknown>>("get_tacticalrmm_system_details", {
        connectionId: connection.id,
        externalId: device.external_id,
      });
  ...
  // Combines a client's filtered "unlinked" and "linked" devices into one
  ...
  function computeGroupDevices(connectionId: string, group: TacticalRmmClientDeviceGroupDto) {
    const groupKey = `${connectionId}:${group.client_id}`;
  ...
  // currently keyboard-active client's device list — same
  // isTypingTarget-gated pattern as NinjaPluginSection.tsx/
  // SnipeitPluginSection.tsx, so typing in the filter input (or any other
  // field) never fights with these single-letter bindings. Only live while
  // the client/device overlay is actually open.
  useEffect(() => {
    function onKeyDown(e: KeyboardEvent) {
      ...
      const activeGroup = activeGroupKey
        ? cached.groups.find((g) => `${openConnectionId}:${g.client_id}` === activeGroupKey)
        : undefined;
      if (!activeGroup) return;
      const { groupKey, navItems } = computeGroupDevices(connection.id, activeGroup);
      // A collapsed client shows no rows at all, so there's nothing for
  ...
  useEffect(() => {
    ...
    setActiveGroupKey((prev) => {
      if (prev && cached.groups.some((g) => `${openConnectionId}:${g.client_id}` === prev)) return prev;
      const firstWithDevices = cached.groups.find((g) => g.devices.length > 0);
      return firstWithDevices ? `${openConnectionId}:${firstWithDevices.client_id}` : null;
    });
  ...
  function renderDetailsPanel(group: TacticalRmmClientDeviceGroupDto, device: ExternalSystemDto, key: string) {
    ...
    return (
      ...
      >
        <div style={{ display: "flex", gap: "0.75rem", alignItems: "center", flexWrap: "wrap", fontSize: "0.8rem", color: "var(--text-secondary)" }}>
          <span>Site (Tactical RMM): {device.site_name ?? <span style={{ color: "var(--text-muted)" }}>—</span>}</span>
          <span>Plattform: {device.platform ? (PLATFORM_LABELS[device.platform] ?? device.platform) : <span style={{ color: "var(--text-muted)" }}>—</span>}</span>
          {device.status && <StatusBadge status={device.status} />}
        </div>
        {busy && <p style={{ margin: 0, fontSize: "0.8rem", color: "var(--text-secondary)" }}>Lade Tactical-RMM-Details…</p>}
              ...
              <table style={{ borderCollapse: "collapse", fontSize: "0.82rem", width: "100%" }}>
                <thead>
                  <tr>
                    ...
                    <th style={thStyle}>Tactical RMM (extern)</th>
                    <th style={thStyle}></th>
                  </tr>
                </thead>
                <tbody>
  ...
  function renderDeviceGroup(connection: TacticalRmmConnectionDto, group: TacticalRmmClientDeviceGroupDto, isFirstFilterable: boolean) {
    ...
    const mapBusyFlag = clientMapBusy[mapKey] ?? false;
    const mapErr = clientMapError[mapKey] ?? null;
    ...
    // mapped) — or the flat filtered list for a not-yet-mapped client,
    ...
    return (
      ...
      >
        ...
        >
          ...
          <span style={{ fontWeight: 600, fontSize: "0.9rem" }}>{group.client_name}</span>
          <span style={{ fontSize: "0.78rem", color: "var(--text-secondary)" }}>
          ...
          <select
            ...
            onChange={(e) => {
              ...
              if (raw === CREATE_NEW_CUSTOMER_VALUE) {
                ...
                // Remember which client is waiting so the
              ...
              void handleClientMappingChange(connection, group, raw);
            }}
          >
        ...
        {isExpanded && (
          <>
            {!isMapped && (
              <p style={mutedStyle}>Dieser Client ist noch keinem Kunden zugeordnet — oben zuordnen, um Geräte zu verknüpfen.</p>
            )}
            {group.devices.length === 0 && <p style={mutedStyle}>Keine Geräte bei diesem Client.</p>}
  ...
  function renderConnectionModal(connection: TacticalRmmConnectionDto) {
    const id = connection.id;
    ...
    return (
      <Modal onClose={closeClientDeviceModal}>
        <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem", minWidth: "32rem", maxWidth: "60rem", maxHeight: "80vh", overflowY: "auto" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: "0.5rem" }}>
            ...
            <button type="button" onClick={closeClientDeviceModal}>
              Schließen
            </button>
          </div>
          ...
          {!cacheBusyFlag && hasCacheLoaded && !cached && (
            ...
            >
              <p style={{ margin: 0, fontSize: "0.85rem", color: "var(--text-secondary)" }}>
                Noch nicht synchronisiert. Auf "Aktualisieren" klicken, um Clients und Geräte von Tactical RMM zu holen.
              </p>
              <button type="button" disabled={busy} onClick={() => void handleSync(connection)}>
                {busy ? "Aktualisiere…" : "Aktualisieren"}
              </button>
          ...
          {cached && cached.groups.length === 0 && <p style={mutedStyle}>Keine Clients im Cache.</p>}
          {cached &&
            (() => {
              const firstFilterableClientId = cached.groups.find((g) => g.devices.length > 0)?.client_id ?? null;
              return cached.groups.map((group) => renderDeviceGroup(connection, group, group.client_id === firstFilterableClientId));
            })()}
        </div>
      </Modal>
    );
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem", maxWidth: "52rem" }}>
      <section style={cardStyle}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: "0.5rem", flexWrap: "wrap" }}>
          <h2 style={{ margin: 0, fontSize: "1rem" }}>Tactical-RMM-Verbindungen</h2>
          <button type="button" onClick={() => setAddFormOpen(true)}>
            + Neue Verbindung
          </button>
        </div>
        <p style={{ margin: 0, color: "var(--text-secondary)", fontSize: "0.85rem" }}>
          Liest ausschließlich Agenten aus Tactical RMM (selbst gehostetes, quelloffenes RMM-Tool) — es werden nie
          Daten dorthin zurückgeschrieben. Eine Verbindung kann mehrere Clients sehen; jeder Client wird einzeln
          ...
          return (
            <div key={connection.id} style={connectionRowStyle}>
              ...
              <div style={{ display: "flex", gap: "0.4rem", flexShrink: 0 }}>
                <button type="button" onClick={() => openClientDeviceModal(connection)}>
                  Clients &amp; Geräte
                </button>
      ...
      {addFormOpen && (
        <Modal onClose={() => setAddFormOpen(false)}>
          <form onSubmit={(e) => void handleAddConnection(e)} style={{ display: "flex", flexDirection: "column", gap: "0.5rem", minWidth: "22rem" }}>
            <h2 style={{ margin: 0, fontSize: "1rem" }}>Neue Tactical-RMM-Verbindung</h2>
            <label style={{ display: "flex", flexDirection: "column", gap: "0.2rem" }}>
              Label
              <input value={newLabel} onChange={(e) => setNewLabel(e.target.value)} placeholder="ACME Tactical RMM" required autoFocus />
            </label>
            <label style={{ display: "flex", flexDirection: "column", gap: "0.2rem" }}>
              Base-URL
              <input
              ...
              <span style={{ fontSize: "0.75rem", color: "var(--text-muted)" }}>
                Die API-Adresse der eigenen, selbst gehosteten Tactical-RMM-Instanz.
              </span>
            </label>
            <label style={{ display: "flex", flexDirection: "column", gap: "0.2rem" }}>
              API-Key
              ...
              />
              <span style={{ fontSize: "0.75rem", color: "var(--text-muted)" }}>
                Erzeugt in Tactical RMM unter Settings → Global Settings → API Keys.
              </span>
            </label>
            ...
            <div style={{ display: "flex", gap: "0.5rem", justifyContent: "flex-end", marginTop: "0.25rem" }}>
              ...
              <button type="button" disabled={testBusy || !newBaseUrl || !newApiKey} onClick={() => void handleTestConnection()}>
