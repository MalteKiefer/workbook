...
// Pulseway (cloud-hosted RMM, pulseway.com, also offered as a self-hosted
// "Enterprise Server" variant under the same API shape) plugin settings
// screen — ninth integration after NinjaOne (NinjaPluginSection.tsx),
// Level.io (LevelPluginSection.tsx), Snipe-IT (SnipeitPluginSection.tsx),
// Microsoft Intune (IntunePluginSection.tsx), Iru (IruPluginSection.tsx),
// Jamf Pro (JamfPluginSection.tsx), Apple Business Manager
// (AbmPluginSection.tsx), and Tactical RMM (TacticalRmmPluginSection.tsx —
// the structural template for this file, see docs/PLUGIN_ARCHITECTURE.md
// "Tactical-RMM-Plugin"). Same read-only/manual-adopt rule as every plugin
...
//
// A connection is NOT tied to a single Kunde, mirroring Ninja/Tactical RMM
// rather than Level: Pulseway models a real "Organization" -> "Site" ->
// "Group" -> "Device" hierarchy (one level deeper than Tactical RMM's
// "Client" -> "Site" -> "Agent") inside one connection (one Base-URL +
// Token-ID/Token-Secret pair), and a single connection can see multiple
// Organizations — a realistic MSP scenario is one shared Pulseway account
...
// Organization being mapped. Mapping granularity deliberately stops at the
// Organization level, NOT Site/Group — an Organization can have multiple
// Sites/Groups, but this integration doesn't add a second mapping layer for
// those (see plugin::pulseway module docs); a device's Site/Group are still
// shown as read-only, informational context (device.site_name/group_name).
//
// Important, verified difference from Tactical RMM (see
// docs/PLUGIN_ARCHITECTURE.md "Pulseway-Plugin" and plugin::pulseway's own
// module docs): Pulseway's device list API DOES carry a genuine numeric
// OrganizationId foreign key, so the backend groups devices by ID (like
// Ninja), not Tactical RMM's name-based workaround — this file just
// consumes the resulting PulsewayOrgDeviceGroupDto[] the same way
// TacticalRmmPluginSection.tsx consumes its client groups. Also unlike
// Tactical RMM, a Pulseway device's list entry has NO ip_address/status/
// platform field at all — Pulseway's own API reference documents those as
// present ONLY on the single-device detail endpoint (used by
// get_pulseway_system_details, i.e. the "Details/Aktualisieren" panel
// below), never on the list/sync path (see module docs on the verified
// gap) — so DeviceSummaryLine below shows Site/Group/"Agent installiert"
// instead of a status dot/platform badge/IP address.
//
// There is deliberately no "open in Pulseway" link on a device row, same
// reasoning as Tactical RMM: a real ExternalUrl field exists in Pulseway's
// API, but only on the separate /assets resource, not /devices, and using
// it would need an extra per-device call this plugin doesn't otherwise need
// (see plugin::pulseway module docs).
//
// Devices are fetched cache-first, exactly like every other plugin section:
// get_cached_pulseway_sync (offline, reads the last known sync) vs.
// sync_pulseway_connection (live, also refreshes the cache server-side).
// The cached sync result is also the single source of truth for
// Organizations (organization_id/organization_name/customer_id come
// straight off each cached group) — no separate live "list organizations"
// call from this page (the backend keeps list_pulseway_organizations
// available for symmetry/first-time setup, but this page never calls it),
// so opening the overlay below without ever syncing simply shows a "Noch
// nicht synchronisiert" prompt.
//
// UI structure: the connections list stays inline on the page (identity + a
// couple of buttons per connection). Everything else — organizations,
// Kunde-mapping, and the linked/unlinked device lists — lives behind a
// per-connection "Organisationen & Geräte" button that opens a Modal
// overlay, tracked via local `openConnectionId` state, matching every other
// plugin section's overlay pattern exactly, including the collapsed-by-
// default organization groups, keyboard-navigable (j/k/Enter/l/u) filtered
// device list, and 10-per-page pagination.
//
// Unlike Tactical RMM's single API-key field, a Pulseway connection needs
// TWO secret values (Token ID + Token Secret), exactly like NinjaOne's
// Client-ID/Client-Secret pair (see NinjaPluginSection.tsx's add-connection
// form) — the add-connection dialog below has four fields (Label, Base-URL,
// Token-ID, Token-Secret) instead of Tactical RMM's three.

interface PulsewayConnectionDto {
  id: string;
  label: string;
  base_url: string;
}
...
interface ExternalSystemDto {
  external_id: string;
  name: string;
  // Always the same value as `name` — Pulseway's own API reference
  // documents the device list's `Name` field AS the hostname, there is no
  // separate field (see plugin::pulseway module docs) — exposed as its own
  // property anyway so matchKeyForDevice below works exactly like every
  // other plugin section's hostname-based "link to existing system"
  // heuristic.
  hostname: string | null;
  // Informational only — NOT part of the organization-mapping join (that's
  // organization_id, a real numeric foreign key, resolved backend-side);
  // just this device's own Pulseway Site name for display, since mapping
  // granularity stops at the Organization level (see module docs above).
  site_name: string | null;
  // Informational only, same rationale as site_name — Pulseway's "Group"
  // level, one further subdivision below Site.
  group_name: string | null;
  is_agent_installed: boolean;
  linked_system_id: number | null;
}

interface PulsewayOrgDeviceGroupDto {
...
interface CachedPulsewaySyncDto {
...
// A Pulseway single-device detail JSON (from get_pulseway_system_details,
// GET /devices/{id}) is documented with PascalCase field names — "Name",
// "ExternalIpAddress", etc. (see plugin::pulseway module docs; unlike
// Tactical RMM's lowercase "hostname"/"public_ip") — kept as a small
// candidate-key scan anyway (not a hardcoded single key), the same
// convention as every other plugin section here, in case a future Pulseway
// API revision renames/adds a field. findExternalValue below compares keys
// case-insensitively, so PascalCase and lowercase candidates both work
// regardless of which casing the live API actually returns.
const NAME_KEYS = ["Name", "name"];
const HOSTNAME_KEYS = ["Name", "hostname"];
const IP_KEYS = ["ExternalIpAddress", "ip_address", "ip"];
...
// Heuristic scan of a get_pulseway_system_details() payload: the exact
...
// spirit to TacticalRmmPluginSection.tsx's/NinjaPluginSection.tsx's
// findExternalValue.
...
// Devices come back from the Pulseway API in whatever order the API returns
...
// Small colored dot + label for whether a Pulseway agent is installed on a
// device — the closest Pulseway equivalent to Tactical RMM's online/
// offline/overdue status dot, but a plain boolean (see module docs: no
// online/offline field at all on the list endpoint).
function AgentInstalledBadge({ installed }: { installed: boolean }) {
  const color = installed ? "var(--success)" : "var(--text-muted)";
  const label = installed ? "Agent installiert" : "Kein Agent";
  return (
  ...
  return (
    <span style={{ display: "inline-flex", alignItems: "center", gap: "0.5rem", flexWrap: "wrap" }}>
      <span>
        ...
        {device.group_name && (
          <span style={{ color: "var(--text-muted)", fontSize: "0.85em" }}> · {device.group_name}</span>
        )}
      </span>
      <AgentInstalledBadge installed={device.is_agent_installed} />
    </span>
  );
}

// Pulseway devices have no separate hostname field distinct from `name`
// (see module docs — "Name" IS the hostname) — so, like Tactical RMM/Ninja/
// Level, the suggested match key for "Mit bestehendem System verknüpfen" is
...
export default function PulsewayPluginSection() {
  ...
  const [connections, setConnections] = useState<PulsewayConnectionDto[]>([]);
  ...
  // the same way as orgMapBusy/orgMapError below
  // (`${connectionId}:${organizationId}`). Set when the user picks "+ Neuen
  // Kunden anlegen…"; consumed and cleared by the
  // customerEditorTarget-closed effect further down, which then auto-maps
  // the newly created customer to this organization.
  ...
  const [cachedSyncByConnection, setCachedSyncByConnection] = useState<Record<string, CachedPulsewaySyncDto | null>>({});
  ...
  // Which organizations are expanded — collapsed by default (an organization
  // renders as just a header row: name, Kunde-mapping select, and a
  // device-count summary). Keyed by `${connectionId}:${organizationId}`,
  // same as everything else below. Deliberately NOT an accordion: multiple
  // organizations can be expanded at once, mirroring every other plugin
  // section's organizations/clients/companies.
  ...
  const reloadConnections = useCallback(() => {
    invoke<PulsewayConnectionDto[]>("list_pulseway_connections")
      .then(setConnections)
  ...
  const loadCachedSync = useCallback(async (connection: PulsewayConnectionDto) => {
    const id = connection.id;
    setCacheBusy((prev) => ({ ...prev, [id]: true }));
    setCacheError((prev) => ({ ...prev, [id]: null }));
    try {
      const cached = await invoke<CachedPulsewaySyncDto | null>("get_cached_pulseway_sync", { connectionId: id });
  ...
  function openOrgDeviceModal(connection: PulsewayConnectionDto) {
  ...
  async function handleTestConnection() {
    ...
    setTestResult(null);
    try {
      await invoke("test_pulseway_connection", { baseUrl: newBaseUrl, tokenId: newTokenId, tokenSecret: newTokenSecret });
  ...
  async function handleAddConnection(e: React.FormEvent) {
    ...
    setAddBusy(true);
    try {
      await invoke("add_pulseway_connection", {
        label: newLabel,
        baseUrl: newBaseUrl,
        tokenId: newTokenId,
  ...
  async function handleRemoveConnection(connection: PulsewayConnectionDto) {
    const confirmed = window.confirm(`Pulseway-Verbindung "${connection.label}" wirklich entfernen?`);
    if (!confirmed) return;
    const id = connection.id;
    setRemoveBusy((prev) => ({ ...prev, [id]: true }));
    setConnectionsError(null);
    try {
      await invoke("remove_pulseway_connection", { id });
  ...
  async function handleSync(connection: PulsewayConnectionDto) {
    const id = connection.id;
    setSyncBusy((prev) => ({ ...prev, [id]: true }));
    setSyncStatus((prev) => ({ ...prev, [id]: null }));
    setSyncError((prev) => ({ ...prev, [id]: null }));
    try {
      const groups = await invoke<PulsewayOrgDeviceGroupDto[]>("sync_pulseway_connection", { connectionId: id });
  ...
  async function handleOrgMappingChange(
    connection: PulsewayConnectionDto,
    group: PulsewayOrgDeviceGroupDto,
    rawValue: string,
  ) {
    ...
    try {
      if (rawValue === "") {
        await invoke("unmap_pulseway_organization", { connectionId: connection.id, organizationId: group.organization_id });
      } else {
        const customerId = Number(rawValue);
        await invoke("map_pulseway_organization", {
  ...
  function openLinkPicker(connection: PulsewayConnectionDto, group: PulsewayOrgDeviceGroupDto, device: ExternalSystemDto) {
  ...
  async function confirmLink(connection: PulsewayConnectionDto, device: ExternalSystemDto) {
  ...
  async function createAndLink(connection: PulsewayConnectionDto, group: PulsewayOrgDeviceGroupDto, device: ExternalSystemDto) {
    if (group.customer_id === null) return;
    const customerId = group.customer_id;
    const key = `${connection.id}:${device.external_id}`;
    ...
    setDeviceError((prev) => ({ ...prev, [key]: null }));
    try {
      // A local System has no site/group/agent-installed field of its own —
      // rather than silently losing that Pulseway-native context on
  ...
  async function handleUnlink(connection: PulsewayConnectionDto, device: ExternalSystemDto) {
    if (device.linked_system_id === null) return;
    const key = `${connection.id}:${device.external_id}`;
    setUnlinkBusy((prev) => ({ ...prev, [key]: true }));
    setDeviceError((prev) => ({ ...prev, [key]: null }));
    try {
      await invoke("unlink_system_from_pulseway", { systemId: device.linked_system_id, connectionId: connection.id });
  ...
  async function toggleDetails(connection: PulsewayConnectionDto, group: PulsewayOrgDeviceGroupDto, device: ExternalSystemDto) {
    const key = `${connection.id}:${device.external_id}`;
    ...
    try {
      const detailsPromise = invoke<Record<string, unknown>>("get_pulseway_system_details", {
        connectionId: connection.id,
        externalId: device.external_id,
      });
  ...
  function computeGroupDevices(connectionId: string, group: PulsewayOrgDeviceGroupDto) {
  ...
  // isTypingTarget-gated pattern as every other plugin section, so typing in
  ...
  function renderDetailsPanel(group: PulsewayOrgDeviceGroupDto, device: ExternalSystemDto, key: string) {
    ...
    return (
      ...
      >
        <div style={{ display: "flex", gap: "0.75rem", alignItems: "center", flexWrap: "wrap", fontSize: "0.8rem", color: "var(--text-secondary)" }}>
          <span>Site (Pulseway): {device.site_name ?? <span style={{ color: "var(--text-muted)" }}>—</span>}</span>
          <span>Gruppe: {device.group_name ?? <span style={{ color: "var(--text-muted)" }}>—</span>}</span>
          <AgentInstalledBadge installed={device.is_agent_installed} />
        </div>
        {busy && <p style={{ margin: 0, fontSize: "0.8rem", color: "var(--text-secondary)" }}>Lade Pulseway-Details…</p>}
              ...
              <table style={{ borderCollapse: "collapse", fontSize: "0.82rem", width: "100%" }}>
                <thead>
                  <tr>
                    ...
                    <th style={thStyle}>Pulseway (extern)</th>
                    <th style={thStyle}></th>
                  </tr>
                </thead>
                <tbody>
  ...
  function renderDeviceGroup(connection: PulsewayConnectionDto, group: PulsewayOrgDeviceGroupDto, isFirstFilterable: boolean) {
      ...
      >
        ...
        {isExpanded && (
          <>
            ...
            {group.devices.length === 0 && <p style={mutedStyle}>Keine Geräte bei dieser Organisation.</p>}
  ...
  function renderConnectionModal(connection: PulsewayConnectionDto) {
    const id = connection.id;
          ...
          {!cacheBusyFlag && hasCacheLoaded && !cached && (
            ...
            >
              <p style={{ margin: 0, fontSize: "0.85rem", color: "var(--text-secondary)" }}>
                Noch nicht synchronisiert. Auf "Aktualisieren" klicken, um Organisationen und Geräte von Pulseway zu holen.
              </p>
              <button type="button" disabled={busy} onClick={() => void handleSync(connection)}>
                {busy ? "Aktualisiere…" : "Aktualisieren"}
              </button>
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem", maxWidth: "52rem" }}>
      <section style={cardStyle}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: "0.5rem", flexWrap: "wrap" }}>
          <h2 style={{ margin: 0, fontSize: "1rem" }}>Pulseway-Verbindungen</h2>
          <button type="button" onClick={() => setAddFormOpen(true)}>
            + Neue Verbindung
          </button>
        </div>
        <p style={{ margin: 0, color: "var(--text-secondary)", fontSize: "0.85rem" }}>
          Liest ausschließlich Geräte aus Pulseway (Cloud-RMM, alternativ ein selbst gehosteter Enterprise Server) — es
          werden nie Daten dorthin zurückgeschrieben. Eine Verbindung kann mehrere Organisationen sehen; jede
          Organisation wird einzeln einem Kunden zugeordnet.
        </p>
      ...
      {addFormOpen && (
        <Modal onClose={() => setAddFormOpen(false)}>
          <form onSubmit={(e) => void handleAddConnection(e)} style={{ display: "flex", flexDirection: "column", gap: "0.5rem", minWidth: "22rem" }}>
            <h2 style={{ margin: 0, fontSize: "1rem" }}>Neue Pulseway-Verbindung</h2>
            <label style={{ display: "flex", flexDirection: "column", gap: "0.2rem" }}>
              Label
              <input value={newLabel} onChange={(e) => setNewLabel(e.target.value)} placeholder="ACME Pulseway" required autoFocus />
            </label>
            <label style={{ display: "flex", flexDirection: "column", gap: "0.2rem" }}>
              Base-URL
              <input
                ...
                placeholder="https://api.pulseway.com/v3"
                style={{ fontFamily: "var(--font-mono)" }}
                required
              />
              <span style={{ fontSize: "0.75rem", color: "var(--text-muted)" }}>
                Standard ist die Pulseway-Cloud (https://api.pulseway.com/v3) — bei einem selbst gehosteten Enterprise
                Server hier stattdessen die eigene Adresse eintragen.
              </span>
            </label>
            <label style={{ display: "flex", flexDirection: "column", gap: "0.2rem" }}>
              Token-ID
            ...
            <label style={{ display: "flex", flexDirection: "column", gap: "0.2rem" }}>
              ...
              />
              <span style={{ fontSize: "0.75rem", color: "var(--text-muted)" }}>
                Vermutlich erzeugbar unter Configuration → API Access → Third Party Tokens → Create Token (Pfad laut
                Pulseways eigener API-Referenz nicht vollständig verifiziert).
              </span>
            </label>
            ...
            <div style={{ display: "flex", gap: "0.5rem", justifyContent: "flex-end", marginTop: "0.25rem" }}>
              ...
              <button
                type="button"
                disabled={testBusy || !newBaseUrl || !newTokenId || !newTokenSecret}
