...
// Opened from SystemListView.tsx's toolbar (scoped to the currently selected
// customer). Combines three independent, unprivileged discovery mechanisms
// -- see this feature's brief for why each is scoped the way it is:
// 1. A blocking TCP-connect sweep over a CIDR range (scan_network).
// 2. A per-host SNMPv1/v2c probe for standard system MIB values (probe_snmp).
// 3. An optional shell-out to a system-installed nmap binary (run_nmap_scan),
//    only offered when is_nmap_available() returns true.
export default function NetworkScanModal({ customerId, onClose, onSystemCreated }: NetworkScanModalProps) {
  ...
  async function handleScan() {
    ...
    setScanBusy(true);
    try {
      const found = await invoke<HostScanResult[]>("scan_network", { cidr });
      setResults(found);
      setCreatedIps(new Set());
      setCreateErrors({});
      setExpandedIp(null);
      if (nmapTarget.trim() === "") setNmapTarget(cidr);
    } catch (e) {
  ...
  async function handleNmapScan() {
    ...
    setNmapOutput(null);
    try {
      const output = await invoke<string>("run_nmap_scan", { target: nmapTarget });
  ...
  return (
    <Modal onClose={onClose}>
      <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem", width: "34rem", maxWidth: "90vw" }}>
        ...
        <div style={{ display: "flex", gap: "0.4rem", alignItems: "flex-end" }}>
          ...
          <button className="btn-primary" onClick={() => void handleScan()} disabled={scanBusy || cidr.trim() === ""}>
            Scannen
          </button>
        </div>
        {scanBusy && (
        ...
        {results.length > 0 && (
          <ul style={{ listStyle: "none", padding: 0, margin: 0, maxHeight: "20rem", overflow: "auto" }}>
            {results.map((result) => (
              <li
                key={result.ip}
                style={{
        ...
        >
          <h3 style={{ margin: 0, fontSize: "0.85rem" }}>nmap</h3>
          {nmapAvailable === false && (
          ...
          {nmapAvailable === true && (
            <>
              <div style={{ display: "flex", gap: "0.4rem", alignItems: "flex-end" }}>
                ...
                <button
                  type="button"
                  onClick={() => void handleNmapScan()}
                  disabled={nmapBusy || nmapTarget.trim() === ""}
                >
                  Mit nmap scannen
                </button>
              </div>
              {nmapBusy && (
                <p style={{ margin: 0, fontSize: "0.82rem", color: "var(--text-secondary)" }}>Scanne…</p>
              )}
