...
export default function SnipeitPluginSection() {
  ...
  // Group-scoped, keyed by `${connectionId}:${company_id}` — disables the
  ...
  async function createAndLinkAll(connection: SnipeitConnectionDto, group: SnipeitCompanyDeviceGroupDto, devices: ExternalSystemDto[]) {
    if (group.customer_id === null || devices.length === 0) return;
    const customerId = group.customer_id;
    const groupKey = `${connection.id}:${group.company_id}`;
