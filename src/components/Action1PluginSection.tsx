...
// Action1 (cloud-hosted, patch-management-focused RMM, action1.com) plugin
// settings screen — structural template is TacticalRmmPluginSection.tsx (see
// docs/PLUGIN_ARCHITECTURE.md "Tactical-RMM-Plugin" and "Action1-Plugin"):
// same read-only/manual-adopt rule as every other plugin section here —
...
// rather than Level/Intune: one Action1 account manages multiple
// "Organizations", and a single connection (region Base-URL + Client ID +
// Client Secret) can see several of them — a realistic MSP scenario is one
// Action1 account managing several customers' endpoints, each as its own
// Organization. So each connection's Organizations are individually mapped
// to a local Kunde, and endpoints are grouped/shown per-Organization, gated
// on that Organization being mapped — structurally identical to Ninja's/
// Tactical RMM's per-Organization/-Client mapping.
//
// IMPORTANT, verified difference from Tactical RMM (see
// docs/PLUGIN_ARCHITECTURE.md "Action1-Plugin" and plugin::action1's own
// module docs): Action1's endpoint objects carry a genuine, verified
// `organization_id` FOREIGN KEY (a UUID string) — unlike Tactical RMM's
// agents, which only carry a flat `client_name` string and need a name-based
// join. The backend already resolves the organization grouping via that ID
// join before this component ever sees the data (an ID join, not a name
// join — see commands::action1::group_endpoints_by_organization), so this
// file just consumes the resulting `Action1OrgDeviceGroupDto[]` the same way
// TacticalRmmPluginSection.tsx consumes its client groups.
//
// Also IMPORTANT: unlike Tactical RMM's per-agent `hostname`/`site_name`
// fields, an Action1 endpoint (as surfaced by ExternalSystemDto here) has no
// separate hostname or site field at all — `name` already prefers the
// endpoint's real `device_name` over its separate, user-editable `name`
// label (see plugin::action1 module docs), so `name` alone doubles as the
// match/compare key wherever Tactical RMM would have used `hostname`.
// Action1 also has no "Site" sub-concept under an Organization the way
// Tactical RMM has "Site" under "Client" — mapping granularity is simply
// per-Organization, nothing finer.
//
// Also IMPORTANT: fetching a single endpoint's detail (or linking one)
// genuinely needs BOTH the organization id and the endpoint id in Action1's
// own URL shape (see plugin::action1 module docs) — unlike every other
// plugin section here, this file therefore always threads `group.organization_id`
// through alongside `device.external_id` when calling
// link_system_to_action1/get_action1_system_details.
//
// There is deliberately no "open in Action1" link on a device row, the same
// honest-omission principle as Tactical RMM's missing dashboard link:
// Action1's web console URL isn't documented anywhere reachable from the
// data this app has.
//
// Endpoints are fetched cache-first, exactly like Ninja/Tactical RMM:
// get_cached_action1_sync (offline, reads the last known sync) vs.
// sync_action1_connection (live, also refreshes the cache server-side). The
...
// cached group) — no separate live "list organizations" call from this page
// (the backend keeps list_action1_organizations available for symmetry/
// first-time setup, but this page never calls it).
//
// UI structure: the connections list stays inline on the page. Everything
...
// opens a Modal overlay, tracked via local `openConnectionId` state,
// matching every other plugin section's overlay pattern exactly, including
// the collapsed-by-default organization groups, keyboard-navigable
// (j/k/Enter/l/u) filtered device list, and 10-per-page pagination.

interface Action1ConnectionDto {
  id: string;
  label: string;
  base_url: string;
}
...
interface ExternalSystemDto {
  ...
  // verbatim from Action1 (see plugin::action1 module docs) — kept as a free
  // string here too, not a union type, so an unrecognized future value still
  // renders instead of failing a type check. This is connectivity —
  // deliberately NOT Action1's separate `online_status` health flag, which
  // this app never surfaces anywhere.
  status: string | null;
  // Action1's platform values aren't a small, verified enum the way Tactical
...
interface Action1OrgDeviceGroupDto {
...
interface CachedAction1SyncDto {
...
// plugin::action1 module docs — the verified endpoint field list). Kept as a
// small candidate-key scan anyway (not a hardcoded single key), the same
// convention as every other plugin section here, in case a future Action1
// API version renames/adds a field.
const NAME_KEYS = ["device_name", "name"];
const HOSTNAME_KEYS = ["device_name"];
const IP_KEYS = ["address", "ip_address", "ip"];
...
// Action1's four fixed regional API hosts (see plugin::action1 module docs —
// each already includes the "/api/3.0" path suffix). A <select> of these is
// friendlier than a free-text field for the common case, while the
// underlying base_url stays a plain string on the backend (Action1ConnectionMeta.base_url),
// so a future fifth region needs only a new entry here, no Rust change.
const REGION_OPTIONS: { value: string; label: string }[] = [
  { value: "https://app.action1.com/api/3.0", label: "Nordamerika (app.action1.com)" },
  { value: "https://app.na-2.action1.com/api/3.0", label: "Nordamerika 2 (app.na-2.action1.com)" },
  { value: "https://app.eu.action1.com/api/3.0", label: "Europa (app.eu.action1.com)" },
  { value: "https://app.au.action1.com/api/3.0", label: "Australien (app.au.action1.com)" },
];

// Human-readable labels for Action1's own verified status enum (see module
// docs on why they're not a Rust/TS enum) — an unrecognized value simply
// falls back to the raw string instead of hiding it.
const STATUS_LABELS: Record<string, string> = {
  Connected: "Verbunden",
  Disconnected: "Getrennt",
  "Pending Uninstall": "Deinstallation ausstehend",
};
const STATUS_COLORS: Record<string, string> = {
  Connected: "var(--success)",
  Disconnected: "var(--text-muted)",
  "Pending Uninstall": "var(--danger)",
};

// Heuristic scan of a get_action1_system_details() payload: the exact shape
...
// Devices come back from the Action1 API in whatever order the API returns
...
// Small colored dot + label for an endpoint's Connected/Disconnected/Pending
// Uninstall status.
...
// An Action1 endpoint's `name` already prefers the real `device_name` over
// the separate, user-editable `name` label (see plugin::action1 module
// docs) — so, like Ninja/Level/Tactical RMM's hostname-based match key, the
// suggested match key for "Link to existing system" is simply the device's
// own `name`, compared against the only free-text field a local system has
// for it — System.hostname.
function matchKeyForDevice(device: ExternalSystemDto): string {
  return (device.name || "").trim().toLowerCase();
}

export default function Action1PluginSection() {
  ...
  const [connections, setConnections] = useState<Action1ConnectionDto[]>([]);
  ...
  const [newBaseUrl, setNewBaseUrl] = useState(REGION_OPTIONS[0].value);
  ...
  // Organization-mapping busy/error, keyed by
  ...
  const [cachedSyncByConnection, setCachedSyncByConnection] = useState<Record<string, CachedAction1SyncDto | null>>({});
  ...
  // organizations can be expanded at once, mirroring Ninja's/Tactical RMM's
  ...
  // Each organization's "Nicht verknüpft" + "Bereits verknüpft" devices are
  ...
  // Group-scoped, keyed by `${connectionId}:${organizationId}` — disables the
  ...
  const reloadConnections = useCallback(() => {
    invoke<Action1ConnectionDto[]>("list_action1_connections")
      .then(setConnections)
  ...
  const loadCachedSync = useCallback(async (connection: Action1ConnectionDto) => {
    const id = connection.id;
    setCacheBusy((prev) => ({ ...prev, [id]: true }));
    setCacheError((prev) => ({ ...prev, [id]: null }));
    try {
      const cached = await invoke<CachedAction1SyncDto | null>("get_cached_action1_sync", { connectionId: id });
  ...
  function openOrgDeviceModal(connection: Action1ConnectionDto) {
  ...
  async function handleTestConnection() {
    ...
    setTestResult(null);
    try {
      await invoke("test_action1_connection", { baseUrl: newBaseUrl, clientId: newClientId, clientSecret: newClientSecret });
  ...
  async function handleAddConnection(e: React.FormEvent) {
    ...
    setAddBusy(true);
    try {
      await invoke("add_action1_connection", {
        label: newLabel,
        baseUrl: newBaseUrl,
      ...
      setNewLabel("");
      setNewBaseUrl(REGION_OPTIONS[0].value);
      setNewClientId("");
      setNewClientSecret("");
      setTestResult(null);
      reloadConnections();
  ...
  async function handleRemoveConnection(connection: Action1ConnectionDto) {
    const confirmed = window.confirm(`Action1-Verbindung "${connection.label}" wirklich entfernen?`);
    if (!confirmed) return;
    const id = connection.id;
    setRemoveBusy((prev) => ({ ...prev, [id]: true }));
    setConnectionsError(null);
    try {
      await invoke("remove_action1_connection", { id });
  ...
  async function handleSync(connection: Action1ConnectionDto) {
    const id = connection.id;
    setSyncBusy((prev) => ({ ...prev, [id]: true }));
    setSyncStatus((prev) => ({ ...prev, [id]: null }));
    setSyncError((prev) => ({ ...prev, [id]: null }));
    try {
      const groups = await invoke<Action1OrgDeviceGroupDto[]>("sync_action1_connection", { connectionId: id });
  ...
  async function handleOrgMappingChange(
    connection: Action1ConnectionDto,
    group: Action1OrgDeviceGroupDto,
    rawValue: string,
  ) {
    ...
    try {
      if (rawValue === "") {
        await invoke("unmap_action1_organization", { connectionId: connection.id, organizationId: group.organization_id });
      } else {
        const customerId = Number(rawValue);
        await invoke("map_action1_organization", {
  ...
  function openLinkPicker(connection: Action1ConnectionDto, group: Action1OrgDeviceGroupDto, device: ExternalSystemDto) {
  ...
  async function confirmLink(connection: Action1ConnectionDto, group: Action1OrgDeviceGroupDto, device: ExternalSystemDto) {
  ...
  async function createAndLink(connection: Action1ConnectionDto, group: Action1OrgDeviceGroupDto, device: ExternalSystemDto) {
    if (group.customer_id === null) return;
    const customerId = group.customer_id;
    const key = `${connection.id}:${device.external_id}`;
    ...
    setDeviceError((prev) => ({ ...prev, [key]: null }));
    try {
      // A local System has no platform field of its own — rather than
      // silently losing that Action1-native context on creation, it's
      ...
      const noteLines = [device.platform ? `Plattform: ${device.platform}` : null].filter(
      ...
      await invoke("sync_action1_connection", { connectionId: connection.id }).catch(() => {});
      await loadCachedSync(connection);
    } catch (err) {
      setDeviceError((prev) => ({ ...prev, [key]: formatInvokeError(err) }));
    } finally {
  ...
  async function createAndLinkAll(connection: Action1ConnectionDto, group: Action1OrgDeviceGroupDto, devices: ExternalSystemDto[]) {
  ...
  async function handleUnlink(connection: Action1ConnectionDto, device: ExternalSystemDto) {
    if (device.linked_system_id === null) return;
    const key = `${connection.id}:${device.external_id}`;
    setUnlinkBusy((prev) => ({ ...prev, [key]: true }));
    setDeviceError((prev) => ({ ...prev, [key]: null }));
    try {
      await invoke("unlink_system_from_action1", { systemId: device.linked_system_id, connectionId: connection.id });
  ...
  async function toggleDetails(connection: Action1ConnectionDto, group: Action1OrgDeviceGroupDto, device: ExternalSystemDto) {
    const key = `${connection.id}:${device.external_id}`;
    ...
    try {
      const detailsPromise = invoke<Record<string, unknown>>("get_action1_system_details", {
        connectionId: connection.id,
        organizationId: group.organization_id,
        externalId: device.external_id,
      });
  ...
  function computeGroupDevices(connectionId: string, group: Action1OrgDeviceGroupDto) {
    ...
    const filteredDevices =
      q === ""
        ...
        : sortedDevices.filter(
            (d) => d.name.toLowerCase().includes(q) || (d.ip_address ?? "").toLowerCase().includes(q),
          );
  ...
  // currently keyboard-active connection's device list — same
  // isTypingTarget-gated pattern as every other plugin section here, so
  // typing in the filter input (or any other field) never fights with these
  ...
  function renderDetailsPanel(group: Action1OrgDeviceGroupDto, device: ExternalSystemDto, key: string) {
    ...
    return (
      ...
      >
        ...
        {busy && <p style={{ margin: 0, fontSize: "0.8rem", color: "var(--text-secondary)" }}>Lade Action1-Details…</p>}
              ...
              <table style={{ borderCollapse: "collapse", fontSize: "0.82rem", width: "100%" }}>
                <thead>
                  <tr>
                    ...
                    <th style={thStyle}>Action1 (extern)</th>
                    <th style={thStyle}></th>
                  </tr>
                </thead>
                <tbody>
  ...
  function renderDeviceGroup(connection: Action1ConnectionDto, group: Action1OrgDeviceGroupDto, isFirstFilterable: boolean) {
                          ...
                          {pickerOpen && (
                            <div style={{ display: "flex", gap: "0.4rem", alignItems: "center", padding: "0.3rem 0.6rem 0" }}>
                              ...
                              <button
                                ...
                                onClick={() => void confirmLink(connection, group, device)}
                              >
  ...
  function renderConnectionModal(connection: Action1ConnectionDto) {
    const id = connection.id;
          ...
          {!cacheBusyFlag && hasCacheLoaded && !cached && (
            ...
            >
              <p style={{ margin: 0, fontSize: "0.85rem", color: "var(--text-secondary)" }}>
                Noch nicht synchronisiert. Auf "Aktualisieren" klicken, um Organisationen und Geräte von Action1 zu holen.
              </p>
              <button type="button" disabled={busy} onClick={() => void handleSync(connection)}>
                {busy ? "Aktualisiere…" : "Aktualisieren"}
              </button>
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem", maxWidth: "52rem" }}>
      <section style={cardStyle}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: "0.5rem", flexWrap: "wrap" }}>
          <h2 style={{ margin: 0, fontSize: "1rem" }}>Action1-Verbindungen</h2>
          <button type="button" onClick={() => setAddFormOpen(true)}>
            + Neue Verbindung
          </button>
        </div>
        <p style={{ margin: 0, color: "var(--text-secondary)", fontSize: "0.85rem" }}>
          Liest ausschließlich Endpunkte aus Action1 (cloud-gehostetes, patch-fokussiertes RMM-Tool) — es werden nie
          Daten dorthin zurückgeschrieben. Eine Verbindung kann mehrere Organisationen sehen; jede Organisation wird
          einzeln einem Kunden zugeordnet.
        </p>
      ...
      {addFormOpen && (
        <Modal onClose={() => setAddFormOpen(false)}>
          <form onSubmit={(e) => void handleAddConnection(e)} style={{ display: "flex", flexDirection: "column", gap: "0.5rem", minWidth: "22rem" }}>
            <h2 style={{ margin: 0, fontSize: "1rem" }}>Neue Action1-Verbindung</h2>
            <label style={{ display: "flex", flexDirection: "column", gap: "0.2rem" }}>
              Label
              <input value={newLabel} onChange={(e) => setNewLabel(e.target.value)} placeholder="ACME Action1" required autoFocus />
            </label>
            <label style={{ display: "flex", flexDirection: "column", gap: "0.2rem" }}>
              Region
              <select
              ...
              <span style={{ fontSize: "0.75rem", color: "var(--text-muted)" }}>
                Die Region des eigenen Action1-Kontos (Configuration → Users and API Credentials).
              </span>
            </label>
            <label style={{ display: "flex", flexDirection: "column", gap: "0.2rem" }}>
              Client-ID
            ...
            <label style={{ display: "flex", flexDirection: "column", gap: "0.2rem" }}>
              ...
              />
              <span style={{ fontSize: "0.75rem", color: "var(--text-muted)" }}>
                Erzeugt im Action1-Web-Konsole unter Configuration → Users and API Credentials.
              </span>
            </label>
            ...
            <div style={{ display: "flex", gap: "0.5rem", justifyContent: "flex-end", marginTop: "0.25rem" }}>
              ...
              <button type="button" disabled={testBusy || !newClientId || !newClientSecret} onClick={() => void handleTestConnection()}>
